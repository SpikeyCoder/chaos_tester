...
# Access Review Policy — website-auditor.io
...
| Google Cloud Platform | OAuth | Yes | Service account key in GitHub Actions |
| Cloudflare | Email + password | Yes | Dashboard access |
| Supabase (psunubqeuopyzgjdytrn) | Dashboard + service role key | Yes | Key in GCP env vars |
| Stripe | Dashboard + API keys | Yes | Keys in GCP env vars |
| Trello | API key + token | N/A | Keys in GCP env vars |
